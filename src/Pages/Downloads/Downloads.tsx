...
import { PDF_DOWNLOADS_FILTER_URL } from "../../Common/Globals";
...
const Downloads = () => {
    ...
    const downloadList = useFetchPdfList({url: PDF_DOWNLOADS_FILTER_URL});

    const downloadsJSX = useMemo(() => {
        if (!downloadList.pdfList) return;
        return downloadList.pdfList.results.map((d, i) => {
            return (
    ...
    return (
        <>
            ...
            <PageLocation img={CommonGlobalExposure} locations={["HOME", "DOWNLOADS"]} title={"DOWNLOADS"}/>
            <div className={"downloads"}>
                {
                    downloadList.reqStates.pending? (
